import torch
from getup_gym.HhdRslRl.basement.base_env.legged_robot import LeggedRobot
import numpy as np
...
def ang_vel_xy(env: LeggedRobot):
        # Penalize xy axes base angular velocity
        return torch.sum(torch.square(env.base_ang_vel[:, :2]), dim=1)

# def heading_cmd(env: LeggedRobot):
def rew_orientation(env: LeggedRobot):
...
def rew_base_height(env: LeggedRobot):
...
def orientation(env: LeggedRobot):
    # 主要：z分量必须接近 -1
...
def termination(env: LeggedRobot):
    # Terminal reward / penalty
    return env.reset_buf * ~env.time_out_buf

def dof_pos_limits(env: LeggedRobot):
...
def hip_limits(env: LeggedRobot):
...
def dof_vel_limits(env: LeggedRobot):
...
def torque_limits(env: LeggedRobot):
...
def tracking_lin_vel(env: LeggedRobot):
...
def tracking_lin_vel_y(env: LeggedRobot):
...
def dof_pos_bias(env: LeggedRobot):
    """在零指令下惩罚运动"""
...
def feet_contact_forces(env: LeggedRobot):
...
def two_leg_bias(env: LeggedRobot):
...
def hip_rate_l2(env: LeggedRobot):
    """hip's reward"""
...
def max_velocity(env: LeggedRobot):
...
def feet_distance(env: LeggedRobot):
...
def wheel_slide_y(env: LeggedRobot):
    """惩罚轮子在y轴方向的滑动"""
...
def wheel_fem_distance_x(env: LeggedRobot):
    """惩罚轮子与FEM之间的距离"""
...
def wheel_velocity_not_equal_to_command(env: LeggedRobot):
    """惩罚轮子速度与指令不一致"""
...
def joint_power(env: LeggedRobot):
    """power pen"""
    base_height = torch.mean(env.root_states[:, 2].unsqueeze(1) - env.measured_heights, dim=1)
    reward = torch.sum(torch.abs(env.dof_vel)* torch.abs(env.torques), dim=1) * (base_height < 0.7)
    return reward

def hip_limit(env: LeggedRobot):
    pen_height = (env.dof_pos[:, 0] < 0.2) | (env.dof_pos[:, 4] > -0.2)
    return pen_height* 1.0

def slow_get_up(env: LeggedRobot):
    up = (
...
def Unitree_feet_distance(env: LeggedRobot):
...
def Unitree_left_foot_displacement(env: LeggedRobot):
...
def Unitree_right_foot_displacement(env: LeggedRobot):
    base_xy = env.root_states[:, :2].clone()
...
def Unitree_ground_parallel(env: LeggedRobot):
...
def Unitree_feet_height_var(env: LeggedRobot):
...
def Unitree_dof_vel(env: LeggedRobot):
        # Penalize dof velocities
    base_height = torch.mean(env.root_states[:, 2].unsqueeze(1) - env.measured_heights, dim=1)
    return torch.sum(torch.square(env.dof_vel), dim=1)
    
def Unitree_shank_orientation(env: LeggedRobot):
...
def Unitree_dof_ang_asymmetry_l1(env: LeggedRobot):
...
def Unitree_dof_ang_asymmetry_l2(env: LeggedRobot):
...
def Unitree_dof_ang_offset_l1(env: LeggedRobot):
...
def Unitree_dof_ang_offset_l2(env: LeggedRobot):
...
def Unitree_no_fly(env: LeggedRobot):
    feet_indices = torch.cat([env.left_foot_body_indices, env.right_foot_body_indices])
    contacts = env.contact_forces[:, feet_indices, 2] > 10.0
    return 1.0 * (torch.sum(1.0 * contacts, dim=1) != 2)

def Unitree_wheel_contact_force(env: LeggedRobot):
    """鼓励轮子接触力的大小"""
    incidces = torch.cat([env.feet_indices, env.hands_indices], dim=-1)
    wheel_force = torch.sum(torch.abs(env.contact_forces[:, incidces, 2]), dim=1)
    total_mass = torch.ones_like(wheel_force)* env.total_mass* 10
    return torch.exp(-torch.abs(wheel_force - total_mass)* 0.05)

def Unitree_gait_feet_frc_perio(env: LeggedRobot) -> torch.Tensor:
...
def Unitree_gait_feet_spd_perio(env: LeggedRobot) -> torch.Tensor:
...
def Unitree_gait_feet_frc_support_perio(env: LeggedRobot) -> torch.Tensor:
...
def Unitree_hip_roll_action(env) -> torch.Tensor:
...
def Unitree_stay_alive(env: LeggedRobot) -> torch.Tensor:
    "reward stay alive"
    return torch.ones(env.num_envs, device=env.device)

def Unitree_hand_support(env: LeggedRobot):
    """奖励手部向下施加力（支撑身体）"""
    # 接触力在z方向的分量（向下为正）
    base_height = torch.mean(env.root_states[:, 2].unsqueeze(1) - env.measured_heights, dim=1)
...
def Unitree_hand_discontact(env: LeggedRobot):
    base_height = torch.mean(env.root_states[:, 2].unsqueeze(1) - env.measured_heights, dim=1)
    left_hand_force = torch.norm(env.contact_forces[:, env.hands_indices[0], :], dim=-1)
    right_hand_force = torch.norm(env.contact_forces[:, env.hands_indices[1], :], dim=-1)
...
def Unitree_ankle_torque(env: LeggedRobot) -> torch.Tensor:
    return torch.sum(torch.square(env.torques[:, env.ankle_joint_indices]), dim=-1)

def Unitree_feet_force_unequal(env: LeggedRobot) -> torch.Tensor:
    base_height = torch.mean(env.root_states[:, 2].unsqueeze(1) - env.measured_heights, dim=1)
...
def Unitree_torso_orientation(env: LeggedRobot) -> torch.Tensor:
    base_height = torch.mean(env.root_states[:, 2].unsqueeze(1) - env.measured_heights, dim=1)
    z_error = torch.square(env.projected_gravity[:, 2] + 1.0)
...
def Unitree_base_vel(env: LeggedRobot) -> torch.Tensor:
    error = torch.square(torch.norm(env.base_lin_vel[:, :], dim=-1))
    base_height = torch.mean(env.root_states[:, 2].unsqueeze(1) - env.measured_heights, dim=1)
    return error* (base_height < 0.6)* base_height 

def Unitree_knee_down(env: LeggedRobot) -> torch.Tensor:
    base_height = torch.mean(env.root_states[:, 2].unsqueeze(1) - env.measured_heights, dim=1)
...
def Unitree_hip_zero(env: LeggedRobot) -> torch.Tensor:
    base_height = torch.mean(env.root_states[:, 2].unsqueeze(1) - env.measured_heights, dim=1)
    error =(
...
def Unitree_feet_xy_force(env: LeggedRobot) -> torch.Tensor:
    base_height = torch.mean(env.root_states[:, 2].unsqueeze(1) - env.measured_heights, dim=1)
...
def Unitree_pelvis_lower_than_base(env: LeggedRobot) -> torch.Tensor:
    base_height = torch.mean(env.rigid_body_states[:, env.base_indices, 2] - env.measured_heights, dim=1)
...
def Unitree_foot_orientation(env: LeggedRobot) -> torch.Tensor:
...
def Unitree_hip_roll_actions(env: LeggedRobot) -> torch.Tensor:
    return torch.sum(
        torch.square(env.actions[:, env.hip_roll_joint_indices]), dim=-1
    ) + torch.sum(torch.square(env.actions[:, env.waist_joint_indices]), dim=-1)

def Unitree_feet_x_alignment(env: LeggedRobot):
    """
    奖励双脚前后位置对齐（x轴）
    目标：双脚并排，x坐标差异越小奖励越高
    """
...
def Unitree_hand_unmove(env: LeggedRobot) -> torch.Tensor:
    """惩罚手部移动"""
    base_height = torch.mean(env.rigid_body_states[:, env.base_indices, 2] - env.measured_heights, dim=1)
